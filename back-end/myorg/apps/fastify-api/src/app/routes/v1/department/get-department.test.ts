...
describe('GET /api/v1/department/:departmentId', () => {
  ...
  it('should return a department when given a valid ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/department/${departments[0].id}`,
      headers: {
  ...
  it('should return 404 when given an invalid ID', async () => {
    ...
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/department/${invalidId}`,
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    });
