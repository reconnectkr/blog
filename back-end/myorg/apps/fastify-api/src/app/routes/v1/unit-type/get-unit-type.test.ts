...
describe('GET /api/v1/unit-type/:unitTypeId', () => {
  ...
  it('should return a unit type when given a valid ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/unit-type/${unitTypes[0].id}`,
      headers: {
    ...
    expect(body).toMatchObject({
      id: unitTypes[0].id,
      name: unitTypes[0].name,
      description: unitTypes[0].description,
    });
  });
  ...
  it('should return 404 when given an invalid ID', async () => {
    ...
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/unit-type/${invalidId}`,
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    });
