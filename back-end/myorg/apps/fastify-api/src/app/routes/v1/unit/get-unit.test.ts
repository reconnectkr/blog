...
describe('GET /api/v1/unit/:unitId', () => {
  ...
  async function createTestFixtures() {
    ...
    units = await prisma.unit.findMany({ orderBy: { id: 'asc' } });
  }
  ...
  it('should return a unit when given a valid ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/unit/${units[0].id}`,
      headers: {
