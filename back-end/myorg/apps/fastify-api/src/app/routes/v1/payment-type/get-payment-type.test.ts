...
describe('GET /api/v1/payment-type/:paymentTypeId', () => {
  ...
  async function createTestFixtures() {
    ...
    paymentTypes = await seedPaymentType(prisma);
  }
  ...
  it('should return a paymentType when given a valid ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/payment-type/${paymentTypes[0].id}`,
      headers: {
  ...
  it('should return 404 when given an invalid ID', async () => {
    ...
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/payment-type/${invalidId}`,
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    });
  ...
  it('should return 400 when given a non-numeric ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: '/api/v1/payment-type/invalid',
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    });
