...
describe('GET /api/v1/price-adjustment-type/:priceAdjustmentTypeId', () => {
  ...
  async function createTestFixtures() {
    ...
    priceAdjustmentTypes = await seedPriceAdjustmentType(prisma);
  }
  ...
  it('should return a priceAdjustmentType when given a valid ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/price-adjustment-type/${priceAdjustmentTypes[0].id}`,
      headers: {
    ...
    expect(body).toMatchObject({
      id: priceAdjustmentTypes[0].id,
      name: priceAdjustmentTypes[0].name,
    });
  });
  ...
  it('should return 404 when given an invalid ID', async () => {
    ...
    const response = await server.inject({
      method: 'GET',
      url: `/api/v1/price-adjustment-type/${invalidId}`,
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    });
  ...
  it('should return 400 when given a non-numeric ID', async () => {
    const response = await server.inject({
      method: 'GET',
      url: '/api/v1/price-adjustment-type/invalid',
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    });
